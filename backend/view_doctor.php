...
<div class="modal fade" id="image-modal" tabindex="-1" role="dialog">
          ...
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card" id="einput">
              <div class="card">
                ...
                <h4 style="background-color: #4B49AC;padding: 10px;color: #fff;border-radius: 5px;">VIEW DOCTORS</h4>
                <br>
  ...
  $result_count = mysqli_query($dbcon,"SELECT COUNT(*) As total_records FROM `doctor`");
  $total_records = mysqli_fetch_array($result_count);
  $total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
  $second_last = $total_no_of_pages - 1; // total page minus 1
$counter = 1;
    $result = mysqli_query($dbcon,"SELECT * FROM `doctor` ORDER BY id DESC LIMIT $offset, $total_records_per_page");
    while($row = mysqli_fetch_array($result)){
 if ($row['status']=="") {
                      $status = "<span style='color: red;'>Not Diagnose</span>";
                  }else{
                    $status = "<span style='color: green;'>Diagnosed</span>";
                  }
                echo 
                "<tr>
                <td>".$counter++."</td>
                  ...
                  <td>".$row['lga']."</td>
                  <td>".$row['email']."</td>
                  <td><a href='delete_doctor.php?id=".$row['id']."'><button class='btn btn-danger btn-sm'>Delete</button></a></td>
                </tr>";
              }
            ?>
